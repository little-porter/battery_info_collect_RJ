...
uint16_t adc2_val[3];
uint16_t adc3_val[FFT_COUNT][2];
uint16_t adc4_val;
uint16_t adc_co;
uint8_t  co_full_flag = 0;
...
void adc_gpio_config(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_10 | GPIO_Pin_11  \
								  | GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15;					//AD_CO;adc_V-;adc_V+;adc_sr101;adc_sr102
...
void adc3_dma2_ch5_config(void)
{
	...
	DMA_InitStructure.DMA_BufferSize = FFT_COUNT*2;
...
void adc2_dma1_ch2_config(void)
{
	DMA_InitTypeDef        DMA_InitStructure;
//	NVIC_InitTypeDef      NVIC_InitStructure;
	/* Enable DMA1 clock */
...
void DMA2_Channel5_IRQHandler(void)
{
	...
	if(DMA_GetITStatus(DMA2_IT_TC5) != RESET)
	{
		...
		{
			battery_inres_window_ad_full((uint16_t *)adc3_val,FFT_COUNT);
